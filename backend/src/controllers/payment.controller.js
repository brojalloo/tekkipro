...
const { PLAN_LIMITS } = require('./abonnement.controller');
...
const createStripeSession = async (req, res) => {
  try {
    if (!stripe) {
      return res.status(503).json({ success: false, message: 'Stripe non configuré. Ajoutez STRIPE_SECRET_KEY dans .env' });
    }
    ...
    res.status(500).json({ success: false, message: 'Erreur lors de la création de la session Stripe' });
  }
};
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (paiement && paiement.statut === 'EN_ATTENTE') {
        ...
        try {
          ...
          console.error('Erreur envoi email confirmation paiement:', emailErr.message);
        }
      }
    }
...
const verifyStripeSession = async (req, res) => {
  try {
    ...
    console.error('Erreur verifyStripeSession:', error);
    res.status(500).json({ success: false, message: 'Erreur vérification session' });
  }
};
...
const initiateWavePayment = async (req, res) => {
  try {
    ...
    if (!telephone) {
      return res.status(400).json({ success: false, message: 'Numéro de téléphone requis pour Wave' });
    }
    ...
    console.error('Erreur initiateWavePayment:', error);
    res.status(500).json({ success: false, message: 'Erreur paiement Wave' });
  }
};
...
const initiateOrangeMoneyPayment = async (req, res) => {
  try {
    ...
    if (!telephone) {
      return res.status(400).json({ success: false, message: 'Numéro de téléphone requis pour Orange Money' });
    }
    ...
    if (process.env.ORANGE_MONEY_API_KEY) {
      ...
      return res.status(503).json({
        success: false,
        message: 'Intégration Orange Money API en cours de développement',
      });
    }
    ...
    console.error('Erreur initiateOrangeMoneyPayment:', error);
    res.status(500).json({ success: false, message: 'Erreur paiement Orange Money' });
  }
};
...
const initiateFreeMoneyPayment = async (req, res) => {
  try {
    ...
    console.error('Erreur initiateFreeMoneyPayment:', error);
    res.status(500).json({ success: false, message: 'Erreur paiement Free Money' });
  }
};
